...
def make_routes_with_water_transport(A, B, _vehicle):
        ...
        'waypoints': [
            {
                ...
                "color": config.FOOT_COLOR if _vehicle == 'foot' else config.BIKE_COLOR,
            },
            {
    ...
    if _vehicle == 'foot' and multi_route_A.get("points") and multi_route_B.get("points"):
        ...
        route = {
            'waypoints': multi_route_A["waypoints"] + river_waypoints + multi_route_B["waypoints"],
            'dist': multi_route_A['dist'] + len(river_route) * 78 + multi_route_B['dist'],
            'time': multi_route_A['time'] + len(river_route) * 78 / 3 * 1000 + multi_route_B['time'],
        ...
        if routes[0]["dist"] * 1.34 > dist_bike:
            routes.append(route)
